...
for people in peopleWithScores{
    for _ in people{
        if highestScore < Int(people["score"] ?? "") ?? 0{
            highestScore = Int(people["score"] ?? "") ?? 0
            highestScoringName = (people["firstName"] ?? "") + " " + (people["lastName"] ?? "")
        }
    }
}
